...
(defun my-create-terminal-function (tuiapp)
  ...
  (let ((function-name (intern (format "%s-term-open" tuiapp))))
    (if (executable-find tuiapp)
        (eval `(defun ,function-name ()
                 ,(format "Open %s using ansi-term." tuiapp)
                 (interactive)
                 (ansi-term ,tuiapp))
              (message "Warning: '%s' tui app not found." tuiapp)))))

(defvar my-tui-apps '("toipe" "nnn" "nmtui" "htop" "nvim")
  "List of tui apps to create functions for.")

(dolist (binary my-tui-apps) (my-create-terminal-function binary))
