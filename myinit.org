...
;; for me site lisp is kind of where i try things out, dump random lisp files
;; and add some of my own packages as git modules
...
(with-eval-after-load 'eshell
  (setq eshell-visual-commands '("vi" "screen" "top" "less" "more" "lynx"
                                 ...
                                 "joe" "jstar" "jmacs" "jpico" "ne" "micro" "nnn" "lf" "ranger" "mc" "gtypist" "toipe"))
...
(defun my-create-terminal-function (tuiapp)
...
(defvar my-tui-apps '("toipe" "nnn" "nmtui" "htop" "nvim" "lynx" "gtypist")
  "List of tui apps to create functions for.")

(add-hook 'after-init-hook
		  (lambda () (dolist (binary my-tui-apps)
					   (my-create-terminal-function binary))))

#+end_src


* COMMENT initialize package managers
...
* COMMENT psudo native (i prefer to install these using the system's package manager)
...
* COMMENT external packages I use
...
(defun create-buttons-from-list (button-list)
  "Create buttons from BUTTON-LIST, where each element is a list containing a button label, description, and action."
    ...
    (let ((button-label (nth 0 item))          ; Get the button label
      ...
      (let ((button (make-button 
                     ...
                     (progn 
                       ...
                       (point)) 
                     'action 
                     button-action))) ; Use the defined action
...
(defun my-dashboard ()
  ...
  ;; Define the ASCII art button locally
...
|    ___||       ||       ||      _||_____  |
|   |___ | ||_|| ||   _   ||     |_  _____| |
|_______||_|   |_||__| |__||_______||_______|

   "))
    
    ;; Create the ASCII art button
    (make-button 
     (point) 
     (progn 
       (insert ascii-art-button) ; Insert the ASCII art for the button
       (point))
     'action 
     (lambda (_) 
  ...
  (let ((buttons
         '(("Scratch Buffer" "Open scratch buffer" 
		   ...
		   ("Org Agenda" "Description of org agenda button" 
    ...
    ;; Use the updated function to create buttons with actions
    (create-buttons-from-list buttons))
