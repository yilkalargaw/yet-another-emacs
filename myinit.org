...
(when EMACS29+
  ...
  (customize-set-variable 'pixel-scroll-precision-large-scroll-height 40.0)
  (customize-set-variable 'pixel-scroll-precision-interpolation-factor 30))
...
(setq outline-minor-mode-list 
      (list '(emacs-lisp-mode "(defun\\|(defvar\\|(defcustom\\|(defconst\\|(defgroup\\|(defmacro")
			...
			;; '(ruby-mode "class \\|[ ]+class \\|module \\|[ ]+module \\|def \\|[ ]+def \\|begin \\|[ ]+begin ")
			;; '(ruby-mode "^\\([ \t]*\\)class \\|^\\([ \t]*\\)module \\|^\\([ \t]*\\)def \\|^\\([ \t]*\\)begin ")
			;; ([:space:]*)
            '(Man-mode "^[A-Z]+")
			'(woman-mode "^[A-Z]+")
            ))
